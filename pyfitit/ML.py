...
class Sample:
    ...
    def getSpectra(self, spType=None, copy=False):
        if spType is None: spType = self.getDefaultSpType()
        if copy: return copy.deepcopy(self._spectra[spType])
        else: return self._spectra[spType]
    ...
    def getSpectrum(self, ind=None, name=None, spType=None, returnIntensityOnly=False):
        ...
        """
        if name is None:
            assert ind is not None, f'You should provide only one argument: ind or name, but not both'
        else:
            assert ind is None, f'You should provide only one argument: ind or name, but not both'
    ...
    def limit(self, energyRange, spType=None, inplace=True):
        ...
        assert len(energy) > 0, f'There are no energy points in the interval {energyRange}. spType={spType}, Energy = {self._energy[spType]}'
    ...
    def decodeAllLabels(self):
        for l in list(self.labelMaps.keys()): self.decode(l)
    ...
    def plot(self, folder, colorParam=None, spType='all spTypes', plotIndividualParams=None, plotSampleParams=None):
        """
        :param plotIndividualParams: dict params of plotToFile to plot individual spectra if plotIndividualParams['plot on sample'] == False. Extra arguments: 'plot on sample': True/False, 'maxIndivPlotCount': number, 'indivIndexes':list, 'indivNames':list
        ...
        if plotIndividualParams is not None:
            ...
            for spType in spTypes:
                if 'maxIndivPlotCount' in plotIndividualParams[spType]:
                    maxIndivPlotCount = plotIndividualParams[spType]['maxIndivPlotCount']
                    del plotIndividualParams[spType]['maxIndivPlotCount']
                else:
                    maxIndivPlotCount = len(self)
                if maxIndivPlotCount>len(self): maxIndivPlotCount=len(self)
                if 'indivIndexes' not in plotIndividualParams[spType] and 'indivNames' not in plotIndividualParams[spType]:
                    indexes = np.arange(maxIndivPlotCount)
                else:
                    if 'indivIndexes' in plotIndividualParams[spType]:
                        indexes = plotIndividualParams[spType]['indivIndexes']
                        del plotIndividualParams[spType]['indivIndexes']
                        for i in indexes: assert i<len(self)
                    else:
                        indexes = [self.getIndByName(nm) for nm in plotIndividualParams[spType]['indivNames']]
                        del plotIndividualParams[spType]['indivNames']
                if 'plot on sample' in plotIndividualParams[spType]:
                    plotOnSample = plotIndividualParams[spType]['plot on sample']
                    del plotIndividualParams[spType]['plot on sample']
                else:
                    plotOnSample = False
                for i in indexes:
...
def distWeightedRankError(y_true, y_pred):
    """
    Measure to estimate ranking quality of distance-based ML methods. y_pred contains distance values. y_true - correct classes (true - 1, wrong - 0).
    Returns (med-true)/(med-0), where med=median(y_pred), true=np.mean(y_pred[y_true==1]). 
        1 - the best (when true=0), 0 - true in the middle, <0 - true beyond the middle
    """
    assert len(y_pred) == len(y_true)
    med = np.median(y_pred)
    true = np.mean(y_pred[y_true==1])
    return (med - true) / med 
...
def validateNormByRef(expSample:Sample, theorySample:Sample, normFunc, spType, plotNormValuesFileName=None):
    """
    Returns distWeightedRankError for ordering theorySample by normFunc to each exp spectrum. Also returns list of commonNames of theory and exp spectra
    ...
    for i,exp_name in enumerate(commonNames):
        ...
        exp_sp = expSample.getSpectrum(ind=i_exp, spType=spType)
        ...
        for j_theor in range(theorySample.getLength()):
            theorySpectrum = theorySample.getSpectrum(ind=j_theor, spType=spType)
        ...
        qualities[i] = distWeightedRankError(true_classes, norm_vals)
    if plotNormValuesFileName is not None:
        plotting.plotMatrix(all_norm_vals, ticklabelsX=theoryNames, ticklabelsY=commonNames, fileName=plotNormValuesFileName, xlabel='theory', ylabel='exp', title=f'Mean quality={np.mean(qualities):.4f}')
...
def validateNormByClasses(expSample:Sample, theorySample:Sample, normFunc, classColumn, spType, plotNormValuesFileName=None):
    """
    Returns distWeightedRankError for ordering theorySample by normFunc to each exp spectrum. Classes of exp and theory spectra can be duplicate
    :param normFunc: func(expSpectrum, theorySpectrum, expParams, theoryParams)
