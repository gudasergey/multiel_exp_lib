...
def chooseFormat(ticks):
    n = len(ticks)
    assert len(np.unique(ticks)) == n, f'Equal ticks detected: {ticks}'
    i = 2
    while True:
        format = f'%.{i}g'
        ticks1 = [format % t for t in ticks]
        if len(ticks1) == n: break
        i += 1
        if i > 18: break
    return format


def addColorBar(mappable, fig, ax, labelMaps, label, ticksPos, ticks, format=None, colorBarLabel=None):
    ...
    if label in labelMaps:
        ...
        if isinstance(ticks[0], float):
            if format is None: format = chooseFormat(ticks)
...
def parseColorMap(cmap):
    ...
    """
    if isinstance(cmap, str) and 'seaborn' in cmap:
